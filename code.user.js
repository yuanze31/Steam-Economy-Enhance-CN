...
// @namespace   https://github.com/yuanze31
// @author      yuanze31
// @license     MIT
// @version     6.9.3
// @description Steam库存市场增强增加各种便捷功能
...
// @grant       unsafeWindow
// @homepageURL https://github.com/yuanze31/Steam-Economy-Enhance-CN
// @supportURL  https://github.com/yuanze31/Steam-Economy-Enhance-CN/issues
// @downloadURL https://raw.githubusercontent.com/yuanze31/Steam-Economy-Enhance-CN/master/code.user.js
// @updateURL   https://raw.githubusercontent.com/yuanze31/Steam-Economy-Enhance-CN/master/code.user.js
// ==/UserScript==
...
(function ($, async) {
  ...
  var country =
    typeof unsafeWindow.g_strCountryCode !== "undefined"
  ...
  var market = new SteamMarket(
    ...
    unsafeWindow.g_strInventoryLoadURL != null
      ...
      : typeof unsafeWindow.g_strProfileURL !== "undefined" &&
        unsafeWindow.g_strProfileURL != null
  ...
  SteamMarket.prototype.sellItem = function (
    item,
    price,
    callback /*err, data*/
  ) {
  ...
  SteamMarket.prototype.removeListing = function (
    item,
    callback /*err, data*/
  ) {
    var sessionId = readCookie("sessionid");
    $.ajax({
      ...
      url:
        ...
        "//steamcommunity.com/market/removelisting/" +
        item,
      data: {
        sessionid: sessionId,
      },
  ...
  SteamMarket.prototype.getPriceIncludingFees = function (price, item) {
    ...
    price = Math.round(price);
    var feeInfo = CalculateAmountToSendForDesiredReceivedAmount(
      price,
      publisherFee,
      this.walletInfo
    );
  ...
  function CalculateFeeAmount(amount, publisherFee, walletInfo) {
    ...
    var nEstimatedAmountOfWalletFundsReceivedByOtherParty = parseInt(
      (amount - parseInt(walletInfo["wallet_fee_base"])) /
        (parseFloat(walletInfo["wallet_fee_percent"]) +
  ...
  if (currentPage == PAGE_INVENTORY) {
    ...
    function updateTotals() {
      ...
      if (totalScrap > 0) {
        totals.innerHTML +=
          "<div><strong>Total scrap " + totalScrap + ".</strong></div>";
      }
    }
    ...
    var sellQueue = async.queue(function (task, next) {
      market.sellItem(task.item, task.sellPrice, function (err, data) {
        ...
        var padLeft =
          padLeftZero("" + totalNumberOfProcessedQueueItems, digits) +
          " / " +
        ...
        if (!err) {
          logDOM(
            padLeft +
              ...
              itemName +
              " 价值" +
              (market.getPriceIncludingFees(task.sellPrice) / 100.0).toFixed(
                2
              ) +
              currencySymbol +
              "，你将收到" +
              (task.sellPrice / 100.0).toFixed(2) +
              currencySymbol +
              ""
          );
          ...
          totalPriceWithFeesOnMarket += market.getPriceIncludingFees(
            task.sellPrice
          );
          updateTotals();
          ...
          } else logDOM(padLeft + " - " + itemName + " not added to market.");
      ...
      loadAllInventories().then(
        function () {
          ...
          filteredItems = marketableItems.filter(
            (e, i) =>
              marketableItems.map((m) => m.classid).indexOf(e.classid) !== i
          );
        ...
        function () {
          ...
          if (numberOfQueuedItems > 0) {
            ...
            $("#inventory_sell_buttons").append(
              '<div id="inventory_items_spinner">' +
                spinnerBlock +
                '<div style="text-align:center">正在处理' +
                numberOfQueuedItems +
                "件物品</div>" +
                "</div>"
            );
    ...
    var scrapQueue = async.queue(function (item, next) {
      scrapQueueWorker(item, function (success) {
        if (success) {
          ...
          if (numberOfFailedRequests > 3) numberOfFailedRequests = 0;
    ...
    function scrapQueueWorker(item, callback) {
      ...
      market.getGooValue(item, function (err, goo) {
        ...
        if (err != ERROR_SUCCESS) {
          ...
          logDOM(
            padLeft +
              " - " +
              itemName +
              " not turned into gems due to missing gems value."
          );

          $("#" + item.appid + "_" + item.contextid + "_" + itemId).css(
            "background",
            COLOR_ERROR
          );
        ...
        market.grindIntoGoo(item, function (err, result) {
          if (err != ERROR_SUCCESS) {
            ...
            logDOM(
              padLeft +
                " - " +
                itemName +
                " not turned into gems due to unknown error."
            );
      ...
      getInventorySelectedMarketableItems(function (items) {
        ...
        items.forEach(function (item) {
          itemsWithQty[item.market_hash_name] =
            itemsWithQty[item.market_hash_name] + 1 || 1;
        });
        ...
        for (var itemName in itemsWithQty) {
          itemsString +=
            ...
            "&qty[]=" +
            itemsWithQty[itemName];
        }

        var baseUrl = "https://steamcommunity.com/market/multisell";
        var redirectUrl =
          baseUrl + "?appid=" + appid + "&contextid=" + contextid + itemsString;

        var dialog = unsafeWindow.ShowDialog(
          "Steam库存市场增强器",
          '<iframe frameBorder="0" height="650" width="900" src="' +
            redirectUrl +
            '"></iframe>'
        );
        ...
        function (err, history, cachedHistory) {
          ...
          market.getItemOrdersHistogram(
            ...
            function (err, histogram, cachedListings) {
              ...
              var sellPrice = calculateSellPriceBeforeFees(
                history,
                histogram,
                true,
    ...
    function initializeInventoryUI() {
      var isOwnInventory =
        unsafeWindow.g_ActiveUser.strSteamId == unsafeWindow.g_steamID;
      ...
      $("#inventories").selectable({
        ...
        selecting: function (e, ui) {
          ...
          var selectedIndex = $(ui.selecting.tagName, e.target).index(
            ui.selecting
          );
      ...
      if (typeof unsafeWindow.CInventory !== "undefined") {
        ...
        unsafeWindow.CInventory.prototype.SelectItem = function (
          event,
          elItem,
          rgItem,
          bUserAction
        ...
        if (items.length == 0) {
          ...
          if (canSellSelectedItemsManually(items)) {
            ...
            $(".sell_manual > span").text(
              "手动出售" +
                selectedItems +
                (selectedItems == 1 ? "个物品" : "个物品")
            );
          } else {
            $(".sell_manual").hide();
          }
          $(".sell_selected > span").text(
            "出售" + selectedItems + (selectedItems == 1 ? "个物品" : "个物品")
          );
        }
      });
    }
    ...
    function updateOpenBoosterPacksButton() {
      getInventorySelectedBoosterPackItems(function (items) {
        ...
        if (items.length == 0) {
          ...
          $(".unpack_booster_packs > span").text(
            "Unpack " +
              selectedItems +
              (selectedItems == 1 ? " Booster Pack" : " Booster Packs")
          );
        }
      });
    }
    ...
    function updateInventorySelection(selectedItem) {
      ...
      market.getItemOrdersHistogram(item, false, function (err, histogram) {
        if (err) {
          logConsole(
            "Failed to get orders histogram for " +
        ...
        $(
          "#" +
            item_info_id +
            "_item_market_actions > div:nth-child(1) > div:nth-child(1)"
        ).hide();

        var isBoosterPack = selectedItem.name
          .toLowerCase()
          .endsWith("booster pack");
        if (isBoosterPack) {
          var tradingCardsUrl =
            "/market/search?q=&category_753_Game%5B%5D=tag_app_" +
            selectedItem.market_fee_app +
            "&category_753_item_class%5B%5D=tag_item_class_2&appid=753";
          ownerActions.append(
            '<br/> <a class="btn_small btn_grey_white_innerfade" href="' +
              tradingCardsUrl +
              '"><span>View trading cards in Community Market</span></a>'
          );
        }
        ...
        $("#" + item_info_id + "_item_market_actions", item_info).append(
          '<div style="display:flex">' +
            '<input id="quick_sell_input" style="background-color: black;color: white;border: transparent;max-width:65px;text-align:center;" type="number" value="' +
            histogram.lowest_sell_order / 100 +
            '" step="0.01" />' +
            '&nbsp;<a class="item_market_action_button item_market_action_button_green quick_sell_custom">' +
            '<span class="item_market_action_button_edge item_market_action_button_left"></span>' +
            '<span class="item_market_action_button_contents">一键出售</span>' +
            '<span class="item_market_action_button_edge item_market_action_button_right"></span>' +
            '<span class="item_market_action_button_preload"></span>' +
            "</a>" +
            "</div>"
        ...
        $(".quick_sell_custom").on("click", function () {
          var price =
            $(
              "#quick_sell_input",
              $("#" + item_info_id + "_item_market_actions", item_info)
            ).val() * 100;
          price = market.getPriceBeforeFees(price);
    ...
    function updateInventoryUI(isOwnInventory) {
      ...
      $("#global_action_menu").prepend(
        '<span id="see_settings"><a href="javascript:void(0)">⬖ Steam库存市场增强器</a></span>'
      );
      ...
      var sellButtons = $(
        '<div id="inventory_sell_buttons" style="margin-bottom:12px;">' +
          ...
          '<a class="btn_green_white_innerfade btn_medium_wide sell_all_duplicates separator-btn-right"><span>出售重复物品</span></a>' +
          '<a class="btn_green_white_innerfade btn_medium_wide sell_selected separator-btn-right" style="display:none"><span>出售所选</span></a>' +
          '<a class="btn_green_white_innerfade btn_medium_wide sell_manual separator-btn-right" style="display:none"><span>手动出售</span></a>' +
          (showMiscOptions
            ? '<a class="btn_green_white_innerfade btn_medium_wide sell_all_cards separator-btn-right"><span>出售所有卡片</span></a>' +
              '<div style="margin-top:12px;">' +
              '<a class="btn_darkblue_white_innerfade btn_medium_wide turn_into_gems separator-btn-right" style="display:none"><span>转换选定物品成宝石</span></a>' +
              '<a class="btn_darkblue_white_innerfade btn_medium_wide gem_all_duplicates separator-btn-right"><span>转换重复物品成宝石</span></a>' +
              '<a class="btn_darkblue_white_innerfade btn_medium_wide unpack_booster_packs separator-btn-right" style="display:none"><span>打开选定加速包</span></a>' +
              "</div>"
            : "") +
          (TF2
            ? '<a class="btn_green_white_innerfade btn_medium_wide sell_all_crates separator-btn-right"><span>出售所有箱子（CS）</span></a>'
            : "") +
          "</div>"
      );

      var reloadButton = $(
        '<a id="inventory_reload_button" class="btn_darkblue_white_innerfade btn_medium_wide reload_inventory" style="margin-right:12px; width: 120px; text-align: center;"><span>重新加载</span></a>'
      );
      ...
      $("#logger").on("scroll", function () {
        var hasUserScrolledToBottom =
          $("#logger").prop("scrollHeight") -
            $("#logger").prop("clientHeight") <=
    ...
    function getInventoryItems() {
      ...
      for (var child in getActiveInventory().m_rgChildInventories) {
        for (var key in getActiveInventory().m_rgChildInventories[child]
          .m_rgAssets) {
          var value =
            getActiveInventory().m_rgChildInventories[child].m_rgAssets[key];
      ...
      market.getItemOrdersHistogram(
        ...
        function (err, histogram, cachedListings) {
          ...
          var itemPrice =
            sellPrice == 65535
              ? "∞"
              : (market.getPriceIncludingFees(sellPrice) / 100.0).toFixed(2) +
                currencySymbol;

          var elementName =
            (currentPage == PAGE_TRADEOFFER ? "#item" : "#") +
            item.appid +
            "_" +
            item.contextid +
            "_" +
          ...
          element.append(
            '<span class="inventory_item_price price_' +
              (sellPrice == 65535
                ? 0
  ...
  if (currentPage == PAGE_MARKET || currentPage == PAGE_MARKET_LISTING) {
        ...
        function (errorPriceHistory, history, cachedHistory) {
          ...
          market.getItemOrdersHistogram(
            ...
            function (errorHistogram, histogram, cachedListings) {
              ...
              var sellPriceWithoutOffsetWithFees = market.getPriceIncludingFees(
                sellPriceWithoutOffset
              );
    ...
    function marketOverpricedQueueWorker(item, ignoreErrors, callback) {
      ...
      market.removeListing(item.listing, function (errorRemove, data) {
        if (!errorRemove) {
          ...
          setTimeout(function () {
            var baseUrl =
              document.querySelector('a.submenuitem[href$="/inventory/"]')
                .href + "json/";
            var itemName = $(".market_listing_item_name_link", listingUI)
              .first()
              .attr("href");
            var marketHashNameIndex = itemName.lastIndexOf("/") + 1;
            var marketHashName = itemName.substring(marketHashNameIndex);
            var decodedMarketHashName = decodeURIComponent(
              itemName.substring(marketHashNameIndex)
            );
            var newAssetId = -1;

            unsafeWindow.RequestFullInventory(
              baseUrl + item.appid + "/" + item.contextid + "/",
              {},
              null,
              null,
              function (transport) {
        ...
        if (!errorRemove) {
          ...
          setTimeout(function () {
            ...
            if (numberOfListings > 0) {
              $("#my_market_selllistings_number").text(
                numberOfListings.toString()
              );

              // This seems identical to the number of sell listings.
              $("#my_market_activelistings_number").text(
                numberOfListings.toString()
              );
            }
          }, 3000);
    ...
    function fillMarketListingsQueue() {
      $(".market_home_listing_table").each(function (e) {
        ...
        if ($(".my_market_header", $(this)).length == 0) return;
    ...
    function processMarketListings() {
      ...
      if (currentPage == PAGE_MARKET) {
        ...
        $(".my_market_header")
          .eq(0)
          .append(
            '<div id="market_listings_spinner">' +
              spinnerBlock +
        ...
        $("#tabContentsMyActiveMarketListingsRows > .market_listing_row").each(
          function () {
            var listingid = $(this)
              .attr("id")
              .replace("mylisting_", "")
            ...
            for (var appid in unsafeWindow.g_rgAssets) {
              for (var contextid in unsafeWindow.g_rgAssets[appid]) {
                for (var assetid in unsafeWindow.g_rgAssets[appid][contextid]) {
                  existingAsset =
                    unsafeWindow.g_rgAssets[appid][contextid][assetid];
                  break;
                }
              }
            }
            ...
            unsafeWindow.g_rgAssets[appid][contextid][assetInfo.assetid] =
              existingAsset;
            marketListingsQueue.push({
              listingid,
              appid: assetInfo.appid,
    ...
    function updateMarketSelectAllButton() {
      $(".market_listing_buttons").each(function () {
        ...
        if ($(".market_select_item", selectionGroup).length == 0)
          ...
          invert = false;
        $(".select_all > span", selectionGroup).text(
          invert ? "取消选择全部" : "选择全部"
        );
      });
    }
    ...
    function sortMarketListings(elem, isPrice, isDate, isName) {
      ...
      $(".market_listing_table_header > span", elem).each(function () {
        if ($(this).hasClass("market_listing_edit_buttons")) return;

        if ($(this).text().includes(arrow_up)) asc = false;
      ...
      if (isPrice) {
        market_listing_selector = $(".market_listing_table_header", elem)
          .children()
          .eq(1);
      } else if (isDate) {
        market_listing_selector = $(".market_listing_table_header", elem)
      ...
      if (isName) {
          ...
          sortFunction: function (a, b) {
            if (
              a
                ...
                .localeCompare(
                  b.values().market_listing_game_name.toLowerCase()
                ) == 0
            ) {
              return a
                .values()
                ...
                .localeCompare(
                  b.values().market_listing_item_name_link.toLowerCase()
                );
            }
            return a
              .values()
        ...
        list.sort("market_listing_listed_date", {
          order: asc ? "asc" : "desc",
          sortFunction: function (a, b) {
            var firstDate = DateTime.fromString(
              a.values().market_listing_listed_date.trim(),
              "d MMM"
            );
            var secondDate = DateTime.fromString(
    ...
    function getListingFromLists(listingid) {
      ...
      for (var i = marketLists.length - 1; i >= 0; i--) {
        ...
        values = marketLists[i].get(
          "market_listing_item_name",
          "mbuyorder_" + listingid + "_name"
        );
        if (values != null && values.length > 0) {
          return values[0];
        }
    ...
    function initializeMarketUI() {
      ...
      $(".my_market_header")
        ...
        .append(
          '<div class="market_listing_buttons">' +
            ...
            '<span class="item_market_action_button_contents" style="text-transform:none">选择前5个</span>' +
            "</a>" +
            '<span class="separator-small"></span>' +
            '<a class="item_market_action_button item_market_action_button_green select_twentyfive_from_page market_listing_button">' +
            '<span class="item_market_action_button_contents" style="text-transform:none">选择前25个</span>' +
            "</a>" +
            '<span class="separator-small"></span>' +
            '<a class="item_market_action_button item_market_action_button_green remove_selected market_listing_button">' +
            '<span class="item_market_action_button_contents" style="text-transform:none">撤下所选</span>' +
            "</a>" +
            ...
            '<span class="item_market_action_button_contents" style="text-transform:none">重新上架所选</span>' +
            "</a>" +
            '<span class="separator-small"></span>' +
            ...
            '<span class="item_market_action_button_contents" style="text-transform:none">重新上架价格过高物品</span>' +
            "</a>" +
            '<span class="separator-small"></span>' +
            ...
            '<span class="item_market_action_button_contents" style="text-transform:none">选择价格过高物品</span>' +
            "</a>" +
            "</div>"
        );
      ...
      $(".my_market_header")
        ...
        .append(
          '<div class="market_listing_buttons">' +
            ...
            '<a class="item_market_action_button item_market_action_button_green remove_selected market_listing_button">' +
            '<span class="item_market_action_button_contents" style="text-transform:none">中止所选</span>' +
            "</a>" +
            "</div>"
        );
      ...
      $(".market_listing_table_header").on("click", "span", function () {
        ...
        var isPrice =
          $(".market_listing_table_header", $(this).parent().parent())
            .children()
            .eq(1)
            .text() == $(this).text();
        var isDate =
      ...
      $(".select_all").on("click", "*", function () {
        ...
        for (var i = 0; i < marketList.matchingItems.length; i++) {
          $(".market_select_item", marketList.matchingItems[i].elm).prop(
            "checked",
            !invert
          );
        }
      ...
      $(".market_relist_auto").change(function () {
        setSetting(
          ...
          $(".market_relist_auto").is(":checked") ? 1 : 0
        );
      });
  ...
  if (currentPage == PAGE_TRADEOFFER) {
    ...
    function getTradeOfferInventoryItems() {
      ...
      for (var child in getActiveInventory().rgChildInventories) {
        for (var key in getActiveInventory().rgChildInventories[child]
          .rgInventory) {
          var value =
            getActiveInventory().rgChildInventories[child].rgInventory[key];
          if (typeof value === "object") {
            // Merges the description in the normal object, this is done to keep the layout consistent with the market page, which is also flattened.
            Object.assign(value, value.description);
            // Includes the id of the inventory item.
    ...
    function sumTradeOfferAssets(assets, user) {
      ...
      for (var i = 0; i < assets.length; i++) {
            ...
            if (inventoryPriceElements.length) {
              ...
              for (var c in classes) {
                if (classes[c].toString().includes("price_")) {
                  var price = parseInt(
                    classes[c].toString().replace("price_", "")
                  );
                  totalPrice += price;
                }
              }
        ...
        if (text in total) total[text] = total[text] + 1;
        else total[text] = 1;
      }

      var sortable = [];
      for (var item in total) sortable.push([item, total[item]]);
      ...
      totalText +=
        "<br/><strong>Total items: " + totalNumOfItems + "</strong><br/>";
  ...
  function hasLoadedAllTradeOfferItems() {
    for (
      var i = 0;
      i < unsafeWindow.g_rgCurrentTradeStatus.them.assets.length;
      i++
    ) {
      var asset = UserThem.findAsset(
        unsafeWindow.g_rgCurrentTradeStatus.them.assets[i].appid,
        unsafeWindow.g_rgCurrentTradeStatus.them.assets[i].contextid,
        unsafeWindow.g_rgCurrentTradeStatus.them.assets[i].assetid
      );
      if (asset == null) return false;
    }
    for (
      var i = 0;
      i < unsafeWindow.g_rgCurrentTradeStatus.me.assets.length;
      i++
    ) {
      var asset = UserYou.findAsset(
        unsafeWindow.g_rgCurrentTradeStatus.me.assets[i].appid,
        unsafeWindow.g_rgCurrentTradeStatus.me.assets[i].contextid,
        unsafeWindow.g_rgCurrentTradeStatus.me.assets[i].assetid
      );
      if (asset == null) return false;
    }
    return true;
  ...
  function initializeTradeOfferUI() {
    ...
    $(".trade_right > div > div > div > .trade_item_box").observe(
      "childlist subtree",
      function (record) {
        if (!hasLoadedAllTradeOfferItems()) return;

        var currentTradeOfferSum =
          unsafeWindow.g_rgCurrentTradeStatus.me.assets.length +
          unsafeWindow.g_rgCurrentTradeStatus.them.assets.length;
        ...
        var your_sum = sumTradeOfferAssets(
          unsafeWindow.g_rgCurrentTradeStatus.me.assets,
          UserYou
        );
        var their_sum = sumTradeOfferAssets(
          unsafeWindow.g_rgCurrentTradeStatus.them.assets,
          UserThem
        );

        $("div.offerheader:nth-child(1) > div:nth-child(3)").append(
          '<div class="trade_offer_sum" id="trade_offer_your_sum">' +
            your_sum +
            "</div>"
        );
        $("div.offerheader:nth-child(3) > div:nth-child(3)").append(
          '<div class="trade_offer_sum" id="trade_offer_their_sum">' +
            their_sum +
            "</div>"
        );
      }
    ...
    if (!window.location.href.includes("tradeoffer/new")) return;
    ...
    $(".select_all").on("click", "*", function () {
      $(
        ".inventory_ctn:visible > .inventory_page:visible > .itemHolder:visible"
      ).delayedEach(250, function (i, it) {
        var item = it.rgItem;
  ...
  function openSettings() {
    var price_options = $(
      '<div id="price_options">' +
        ...
        '将价格设置为：&nbsp;<select class="price_option_input" style="background-color: black;color: white;border: transparent;" id="' +
        ...
        (getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 1
          ? 'selected="selected"'
          : "") +
        ">成交均价和最低出售价中的最大值</option>" +
        '<option value="2" ' +
        (getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 2
          ? 'selected="selected"'
          : "") +
        ">最低出售价</option>" +
        '<option value="3" ' +
        (getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 3
          ? 'selected="selected"'
          : "") +
        ">最高求购价或最低出售价</option>" +
        "</select>" +
        ...
        "</div>" +
        '<div style="margin-bottom:6px;">' +
        '计算最近几小时的成交均价：&nbsp;<input class="price_option_input" style="background-color: black;color: white;border: transparent;" type="number" step="2" id="' +
        ...
        "</div>" +
        '<div style="margin-bottom:6px;">' +
        '计算价格时要添加的值（需遵守最小值和最大值限制）：&nbsp;<input class="price_option_input price_option_price" style="background-color: black;color: white;border: transparent;" type="number" step="0.01" id="' +
        SETTING_PRICE_OFFSET +
        '" value=' +
        getSettingWithDefault(SETTING_PRICE_OFFSET) +
        ">" +
        "<br/>" +
        "</div>" +
        '<div style="margin-top:6px">' +
        '当最低售价的数量较少时，使用第二低的售价：&nbsp;<input class="price_option_input" style="background-color: black;color: white;border: transparent;" type="checkbox" id="' +
        ...
        "</div>" +
        '<div style="margin-top:6px;">' +
        '不检查价格低于多少的出售/求购列表：&nbsp;<input class="price_option_input price_option_price" style="background-color: black;color: white;border: transparent;" type="number" step="0.01" id="' +
        SETTING_PRICE_MIN_CHECK_PRICE +
        '" value=' +
        getSettingWithDefault(SETTING_PRICE_MIN_CHECK_PRICE) +
        ">" +
        "<br/>" +
        "</div>" +
        '<div style="margin-top:24px">' +
        '在库存中显示价格标签：&nbsp;<input class="price_option_input" style="background-color: black;color: white;border: transparent;" type="checkbox" id="' +
        SETTING_INVENTORY_PRICE_LABELS +
        '" ' +
        (getSettingWithDefault(SETTING_INVENTORY_PRICE_LABELS) == 1
          ? 'checked=""'
          : "") +
        ">" +
        "</div>" +
        '<div style="margin-top:6px">' +
        '在报价中显示价格标签：&nbsp;<input class="price_option_input" style="background-color: black;color: white;border: transparent;" type="checkbox" id="' +
        SETTING_TRADEOFFER_PRICE_LABELS +
        '" ' +
        (getSettingWithDefault(SETTING_TRADEOFFER_PRICE_LABELS) == 1
          ? 'checked=""'
          : "") +
        ">" +
        "</div>" +
        '<div style="margin-top:24px">' +
        '<div style="margin-bottom:6px;">' +
        '普通卡售价：最低&nbsp;<input class="price_option_input price_option_price" style="background-color: black;color: white;border: transparent;" type="number" step="0.01" id="' +
        SETTING_MIN_NORMAL_PRICE +
        '" value=' +
        getSettingWithDefault(SETTING_MIN_NORMAL_PRICE) +
        ">&nbsp;" +
        '，最高&nbsp;<input class="price_option_input price_option_price" style="background-color: black;color: white;border: transparent;" type="number" step="0.01" id="' +
        ...
        "</div>" +
        '<div style="margin-bottom:6px;">' +
        '闪亮卡售价：最低&nbsp;<input class="price_option_input price_option_price" style="background-color: black;color: white;border: transparent;" type="number" step="0.01" id="' +
        ...
        "</div>" +
        '<div style="margin-bottom:6px;">' +
        '其他物品售价：最低&nbsp;<input class="price_option_input price_option_price" style="background-color: black;color: white;border: transparent;" type="number" step="0.01" id="' +
        SETTING_MIN_MISC_PRICE +
        '" value=' +
        getSettingWithDefault(SETTING_MIN_MISC_PRICE) +
        ">&nbsp;" +
        '，最高;<input class="price_option_input price_option_price" style="background-color: black;color: white;border: transparent;" type="number" step="0.01" id="' +
        SETTING_MAX_MISC_PRICE +
        '" value=' +
        getSettingWithDefault(SETTING_MAX_MISC_PRICE) +
        ">&nbsp;" +
        "<br/>" +
        "</div>" +
        '<div style="margin-top:24px;margin-bottom:6px;">' +
        '市场每页显示多少物品：&nbsp;<input class="price_option_input price_option_price" style="background-color: black;color: white;border: transparent;" type="number" step="0.01" id="' +
        SETTING_MARKET_PAGE_COUNT +
        '" value=' +
        getSettingWithDefault(SETTING_MARKET_PAGE_COUNT) +
        ">" +
        "<br/>" +
        '<div style="margin-top:6px;">' +
        '自动重新发布价格过高的物品（库存过多时速度较慢）：&nbsp;<input id="' +
        SETTING_RELIST_AUTOMATICALLY +
        '" class="market_relist_auto" type="checkbox" ' +
    ...
    var dialog = unsafeWindow
      .ShowConfirmDialog("Steam库存市场增强器", price_options)
      .done(function () {
        setSetting(
          SETTING_MIN_NORMAL_PRICE,
        ...
        setSetting(
          SETTING_PRICE_OFFSET,
          $("#" + SETTING_PRICE_OFFSET, price_options).val()
        );
        setSetting(
          SETTING_PRICE_MIN_CHECK_PRICE,
        ...
        setSetting(
          SETTING_PRICE_IGNORE_LOWEST_Q,
          $("#" + SETTING_PRICE_IGNORE_LOWEST_Q, price_options).prop("checked")
            ? 1
            : 0
        );
        setSetting(
        ...
        setSetting(
          SETTING_MARKET_PAGE_COUNT,
          $("#" + SETTING_MARKET_PAGE_COUNT, price_options).val()
        );
        setSetting(
          SETTING_RELIST_AUTOMATICALLY,
          $("#" + SETTING_RELIST_AUTOMATICALLY, price_options).prop("checked")
            ? 1
            : 0
        );
        setSetting(
          SETTING_INVENTORY_PRICE_LABELS,
          $("#" + SETTING_INVENTORY_PRICE_LABELS, price_options).prop("checked")
            ? 1
            : 0
        );
        setSetting(
          SETTING_TRADEOFFER_PRICE_LABELS,
          $("#" + SETTING_TRADEOFFER_PRICE_LABELS, price_options).prop(
            "checked"
          )
            ? 1
            : 0
