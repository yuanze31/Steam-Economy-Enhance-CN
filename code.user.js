...
(function ($, async) {
    ...
    var isLoggedIn =
        (typeof unsafeWindow.g_rgWalletInfo !== "undefined" && unsafeWindow.g_rgWalletInfo != null) ||
    ...
    var currencyId =
        isLoggedIn && market != null && market.walletInfo != null && market.walletInfo.wallet_currency != null
        ...
        try {
            ...
            storagePersistent
                ...
                .then(function (value) {
                    ...
                    else return market.getCurrentMarketItemNameId(appid, market_name, callback);
                })
                .catch(function (error) {
    ...
    SteamMarket.prototype.getPriceBeforeFees = function (price, item) {
        ...
        if (publisherFee == -1) {
            if (this.walletInfo != null) publisherFee = this.walletInfo["wallet_publisher_fee_percent_default"];
    ...
    function getMarketHashName(item) {
        ...
        if (item.description != null && item.description.market_hash_name != null) return escapeURI(item.description.market_hash_name);

        if (item.description != null && item.description.name != null) return escapeURI(item.description.name);
    ...
    function getIsCrate(item) {
        ...
        if (tags != null) {
            ...
            tags.forEach(function (arrayItem) {
                if (arrayItem.category == "Type") if (arrayItem.internal_name == "Supply Crate") isTaggedAsCrate = true;
            });
    ...
    if (currentPage == PAGE_INVENTORY) {
                ...
                selecting: function (e, ui) {
                    ...
                    if (e.shiftKey && previousSelection > -1) {
                        $(ui.selecting.tagName, e.target)
                            .slice(Math.min(previousSelection, selectedIndex), 1 + Math.max(previousSelection, selectedIndex))
                            .each(function () {
                                if ($(this).is(filter)) {
        ...
        function updateTurnIntoGemsButton() {
            getInventorySelectedGemsItems(function (items) {
                ...
                if (items.length == 0) {
                    ...
                    $(".turn_into_gems").show();
                    $(".turn_into_gems > span").text("转换" + selectedItems + (selectedItems == 1 ? "个物品成宝石" : "个物品成宝石"));
                }
            });
        }
        ...
        function updateInventoryUI(isOwnInventory) {
            ...
            $("#see_settings").remove();
            $("#global_action_menu").prepend('<span id="see_settings"><a href="javascript:void(0)">⬖ Steam库存市场增强器</a></span>');
            ...
            $("#logger").on("scroll", function () {
                var hasUserScrolledToBottom =
                    $("#logger").prop("scrollHeight") - $("#logger").prop("clientHeight") <= $("#logger").prop("scrollTop") + 1;
            ...
            market.getItemOrdersHistogram(item, true, function (err, histogram, cachedListings) {
                ...
                element.append(
                    '<span class="inventory_item_price price_' +
                        (sellPrice == 65535 ? 0 : market.getPriceIncludingFees(sellPrice)) +
                        '">' +
                        itemPrice +
                        "</span>"
                );
    ...
    if (currentPage == PAGE_MARKET || currentPage == PAGE_MARKET_LISTING) {
        ...
        function getPriceFromMarketListing(listing) {
            ...
            if (priceLabel[priceLabel.length - 1] === "." || priceLabel[priceLabel.length - 1] === ",") priceLabel = priceLabel.slice(0, -1);
            ...
            market.getPriceHistory(item, true, function (errorPriceHistory, history, cachedHistory) {
                ...
                market.getItemOrdersHistogram(item, true, function (errorHistogram, histogram, cachedListings) {
                    ...
                    var highestBuyOrderPrice =
                        histogram == null || histogram.highest_buy_order == null ? "-" : histogram.highest_buy_order / 100 + currencySymbol;
                    $(".market_table_value > span:nth-child(1) > span:nth-child(1) > span:nth-child(1)", listingUI).append(
                        ' ➤ <span title="This is likely the highest buy order price.">' + highestBuyOrderPrice + "</span>"
                    );
                    ...
                    $(".market_listing_my_price", listingUI)
                        .last()
                        .prop("title", "The best price is " + sellPriceWithoutOffsetWithFees / 100.0 + currencySymbol + ".");
        ...
        function queueOverpricedItemListing(listingid) {
            ...
            for (var i in items) {
                if (items[i].toString().includes("price_")) price = parseInt(items[i].toString().replace("price_", ""));
            }
        ...
        var marketListingsItemsQueue = async.queue(function (listing, next) {
            $.get(
                window.location.protocol + "//steamcommunity.com/market/mylistings?count=100&start=" + listing,
                function (data) {
        ...
        marketListingsItemsQueue.drain = function () {
            ...
            $(".market_listing_row", myMarketListings).each(function () {
                ...
                if ($(".item_market_action_button", this).attr("href").toLowerCase().includes("CancelMarketBuyOrder".toLowerCase())) $(this).remove();
            });
        ...
        function fillMarketListingsQueue() {
            ...
            for (var i = 0; i < marketLists.length; i++) {
                for (var j = 0; j < marketLists[i].items.length; j++) {
                    ...
                    if (!isNaN(assetInfo.priceBuyer)) totalPriceBuyer += assetInfo.priceBuyer;
                    if (!isNaN(assetInfo.priceSeller)) totalPriceSeller += assetInfo.priceSeller;
        ...
        function processMarketListings() {
            ...
            if (currentPage == PAGE_MARKET) {
                ...
                $(".market_home_listing_table").each(function (e) {
                    ...
                    if ($("#market_buyorder_info_show_details", $(this)).length > 0) return;
        ...
        function updateMarketSelectAllButton() {
            $(".market_listing_buttons").each(function () {
                ...
                if ($(".market_select_item", selectionGroup).length == 0)
                    ...
                    invert = false;
                $(".select_all > span", selectionGroup).text(invert ? "取消选择全部" : "选择全部");
            });
        }
        ...
        function getListFromContainer(group) {
            for (var i = 0; i < marketLists.length; i++) {
                if (group.attr("id") == $(marketLists[i].listContainer).attr("id")) return marketLists[i];
            }
        }
        ...
        function initializeMarketUI() {
            ...
            $(".market_listing_table_header").on("click", "span", function () {
                if ($(this).hasClass("market_listing_edit_buttons") || $(this).hasClass("item_market_action_button_contents")) return;
            ...
            $(".select_overpriced").on("click", "*", function () {
                ...
                $(".market_listing_row", selectionGroup).each(function (index) {
                    if ($(this).hasClass("overpriced")) $(".market_select_item", $(this)).prop("checked", true);
                });
    ...
    function openSettings() {
        var price_options = $(
            '<div id="price_options">' +
                ...
                '">' +
                '<option value="1"' +
                (getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 1 ? 'selected="selected"' : "") +
                ">成交均价和最低出售价中的最大值</option>" +
                '<option value="2" ' +
                (getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 2 ? 'selected="selected"' : "") +
                ">最低出售价</option>" +
                '<option value="3" ' +
                (getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 3 ? 'selected="selected"' : "") +
                ">最高求购价或最低出售价</option>" +
                "</select>" +
                "<br/>" +
                "</div>" +
                ...
                '" ' +
                (getSettingWithDefault(SETTING_PRICE_IGNORE_LOWEST_Q) == 1 ? 'checked=""' : "") +
                ">" +
                "<br/>" +
                "</div>" +
                '<div style="margin-top:6px;">' +
                ...
                '" ' +
                (getSettingWithDefault(SETTING_INVENTORY_PRICE_LABELS) == 1 ? 'checked=""' : "") +
                ">" +
                "</div>" +
                '<div style="margin-top:6px">' +
                ...
                '" ' +
                (getSettingWithDefault(SETTING_TRADEOFFER_PRICE_LABELS) == 1 ? 'checked=""' : "") +
                ">" +
                "</div>" +
                '<div style="margin-top:24px">' +
                '<div style="margin-bottom:6px;">' +
                ...
                (getSettingWithDefault(SETTING_RELIST_AUTOMATICALLY) == 1 ? 'checked=""' : "") +
                ">" +
                "</label>" +
                "</div>" +
                "</div>" +
        ...
        var dialog = unsafeWindow.ShowConfirmDialog("Steam库存市场增强器", price_options).done(function () {
